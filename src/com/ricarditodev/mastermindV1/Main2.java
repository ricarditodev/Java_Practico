...
public class Main2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < LARGO_CODIGO; i++) {
            ...
            int j = 0; //variable auxiliar para verificar que los numeros no se repiten en el arreglo
            while (j < i) {
                //si pensador en indice de j es igual al numero aleatorio
                //restablecemos j a 0 y volvemos a generar un numero al azar para que lo vuelva a comprobar desde el inicio
                if (pensador[j] == indice) {
                    j = 0;
                    indice = generador.nextInt((int) ULTIMA_LETRA - (int) PRIMERA_LETRA + 1) + (int) PRIMERA_LETRA;
                } else {
                    j++;
